...
public class RequestInventoryItemsDelete extends MessageHandler {
    ...
    public void handle() {
        ...
        for (int i = 0; i < Math.abs(amount); i++) {
            HabboItem habboInvItem = habbo.getInventory().getItemsComponent().getAndRemoveHabboItem(item);
            if (habboInvItem != null)
                toRemove.put(habboInvItem.getId(), habboInvItem);
        }
