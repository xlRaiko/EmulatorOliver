...
import com.eu.habbo.plugin.events.users.catalog.UserCatalogSubscriptionPurchasedEvent;
        ...
        if (Emulator.getIntUnixTimestamp() - this.client.getHabbo().getHabboStats().lastPurchaseTimestamp >= CatalogManager.PURCHASE_COOLDOWN) {
            ...
            if (pageId == -12345678 || pageId == -1) {
                ...
                if (page instanceof RoomBundleLayout) {
                    ...
                    if (roomBundleItem == null || roomBundleItem.getCredits() > this.client.getHabbo().getHabboInfo().getCredits() || roomBundleItem.getPixels() > this.client.getHabbo().getHabboInfo().getPixels() || roomBundleItem.getPoints() > this.client.getHabbo().getHabboInfo().getCurrencyAmount(roomBundleItem.getPointsType())) {
                    ...
                    if (!this.client.getHabbo().hasPermission(Permission.ACC_INFINITE_PIXELS)) { //if the player has this perm disabled
                        this.client.getHabbo().givePixels(-roomBundleItem.getPixels()); // takes their pixels away
                    }
            ...
            if (page instanceof ClubBuyLayout || page instanceof VipBuyLayout) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    totalPixels += item.getPixels();
                    totalPoints += item.getPoints();
                }

                if (totalDays > 0) {
                    if (this.client.getHabbo().getHabboInfo().getCurrencyAmount(item.getPointsType()) < totalPoints)
                        return;

                    if (this.client.getHabbo().getHabboInfo().getCredits() < totalCredits)
                        return;

                    if (this.client.getHabbo().getHabboInfo().getPixels() < totalPixels)
                        return;

                    UserCatalogSubscriptionPurchasedEvent purchasedEvent = new UserCatalogSubscriptionPurchasedEvent(this.client.getHabbo(), item, totalCredits, totalPixels, totalPoints);
                    Emulator.getPluginManager().fireEvent(purchasedEvent);

                    if (!this.client.getHabbo().hasPermission(Permission.ACC_INFINITE_CREDITS))
                        this.client.getHabbo().giveCredits(-totalCredits);

                    if (!this.client.getHabbo().hasPermission(Permission.ACC_INFINITE_PIXELS))
                        this.client.getHabbo().givePixels(-totalPixels);
                    ...
                    if (item.isVip()) {
                        if(this.client.getHabbo().getHabboStats().createSubscription(Subscription.HABBO_VIP, (totalDays * 86400)) == null) {
                            this.client.sendResponse(new AlertPurchaseFailedComposer(AlertPurchaseFailedComposer.SERVER_ERROR).compose());
