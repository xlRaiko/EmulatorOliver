...
public class SendGift extends RCONMessage<SendGift.SendGiftJSON> {
    ...
    public void handle(Gson gson, SendGiftJSON json) {
        ...
        HabboInfo user = HabboManager.getOfflineHabboInfo(json.user_id);

        UserExecuteRconEvent userExecuteCommandEvent = new UserExecuteRconEvent(user, "gift", new String[]{ "Item", String.valueOf(item.getBaseItem())});
