...
public class SuperbanCommand extends Command {
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        ...
        if (target == null) {
            ...
            Emulator.getGameEnvironment().getModToolManager().ban(target.getId(), gameClient.getHabbo(), reason.substring(0, reason.length()-1), minutes, ModToolBanType.IP, -1);
            for (Habbo h : Emulator.getGameServer().getGameClientManager().getHabbosWithIP(target.getIpLogin())) {
                if (h != null) {
                    Emulator.getGameEnvironment().getModToolManager().ban(h.getHabboInfo().getId(), gameClient.getHabbo(), reason.substring(0, reason.length()-1), minutes, ModToolBanType.IP, -1);
                }
            }

            gameClient.getHabbo().whisper(Emulator.getTexts().getValue("commands.success.cmd_super_ban").replace("%username%", target.getUsername()).replace("%reason%", reason).replace("%minutes%", initial), RoomChatMessageBubbles.ALERT);
        }
