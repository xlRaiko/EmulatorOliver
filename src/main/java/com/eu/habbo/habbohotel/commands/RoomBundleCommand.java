...
public class RoomBundleCommand extends Command {
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        ...
        pixels = Integer.parseInt(params[3]);
        points = Integer.parseInt(params[4]);
        pointsType = Integer.parseInt(params[5]);
        ...
        if (page instanceof RoomBundleLayout) {
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("INSERT INTO catalog_items (page_id, item_ids, catalog_name, cost_credits, cost_pixels, cost_points, points_type ) VALUES (?, ?, ?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS)) {
                statement.setInt(1, page.getId());
                statement.setString(2, "");
                statement.setString(3, "room_bundle");
                statement.setInt(4, credits);
                statement.setInt(5, pixels);
                statement.setInt(6, points);
                statement.setInt(7, pointsType);
                statement.execute();
            ...
            gameClient.getHabbo().whisper(Emulator.getTexts().getValue("commands.succes.cmd_bundle").replace("%id%", String.valueOf(page.getId())), RoomChatMessageBubbles.ALERT);
        }
