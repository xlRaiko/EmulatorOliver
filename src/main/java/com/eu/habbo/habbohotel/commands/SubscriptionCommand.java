        ...
        if (params.length >= 4) {
            ...
            if (info != null) {
                ...
                if(action.equalsIgnoreCase("add") || action.equalsIgnoreCase("+") || action.equalsIgnoreCase("a")) {
                    ...
                    if (subscription.equals("HABBO_VIP")) {
                        if (info.getRank().getId() == 1) {
                            Emulator.getGameEnvironment().getHabboManager().setRank(info.getId(), 2);
                        }
                    }

                    gameClient.getHabbo().whisper(Emulator.getTexts().getValue("commands.error.cmd_subscription.success_add_time", "Successfully added %time% seconds to %subscription% on %user%").replace("%time%", String.valueOf(timeToAdd)).replace("%user%", params[1]).replace("%subscription%", subscription), RoomChatMessageBubbles.ALERT);
                }
                    ...
                    if(message.length() != 0) {
                        ...
                        gameClient.getHabbo().whisper(Emulator.getTexts().getValue("commands.error.cmd_subscription.success_remove_time", "Successfully removed %time% seconds from %subscription% on %user%").replace("%time%", String.valueOf(timeToRemove)).replace("%user%", params[1]).replace("%subscription%", subscription), RoomChatMessageBubbles.ALERT);
                    }
                    else {
