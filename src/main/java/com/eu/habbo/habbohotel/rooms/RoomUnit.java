...
public class RoomUnit {
    ...
    private boolean superFastWalk = false;
    ...
    public boolean cycle(Room room) {
        try {
            ...
            boolean canSuperFastWalk = true;

            Habbo habboT = room.getHabbo(this);
            if (habboT != null) {
                if (habboT.getHabboInfo().getRiding() != null) {
                    canFastWalk = false;
                    canSuperFastWalk = false;
                }
            }
            ...
            if (canSuperFastWalk && this.superFastWalk) {
                if (this.path.size() > 1) {
                    next = this.path.pollLast();
                }
            }
    ...
    public boolean isSuperFastWalk() {
        return this.superFastWalk;
    }

    public void setSuperFastWalk(boolean superFastWalk) {
        this.superFastWalk = superFastWalk;
    }
