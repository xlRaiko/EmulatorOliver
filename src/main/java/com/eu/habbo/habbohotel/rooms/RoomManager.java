...
public class RoomManager {
    ...
    void openRoom(Habbo habbo, Room room, RoomTile doorLocation) {
        ...
        habbo.getRoomUnit().setSuperFastWalk(habbo.getRoomUnit().isSuperFastWalk() && habbo.hasPermission("cmd_super_fastwalk", room.hasRights(habbo)));
    ...
    public void enterRoom(final Habbo habbo, final Room room) {
        ...
        if (habbo.getHabboStats().cache.containsKey("invisible")) {
            Emulator.getThreading().run(() -> {
                habbo.getRoomUnit().setInvisible(true);
                habbo.getHabboInfo().getCurrentRoom().sendComposer(new RoomUserRemoveComposer(habbo.getRoomUnit()).compose());
            }, 100);
        }

        if (habbo.getHabboStats().cache.containsKey("habbo.transform")) {
            TransformCommand command = new TransformCommand();
            String[] params = habbo.getHabboStats().cache.get("habbo.transform").toString().split(" ");
            command.handle(habbo.getClient(), params);
        ...
        habbo.getClient().sendResponse(new RoomWallItemsComposer(room));
        {
            ...
            allFloorItems.forEach(new TObjectProcedure<HabboItem>() {
                ...
                public boolean execute(HabboItem object) {
                    if (room.isHideWired() && (object instanceof InteractionWired || object instanceof InteractionTileWalkMagic || object.getBaseItem().getFullName().contains("hideable")))
                        return true;
    ...
    public void banUserFromRoom(Habbo rights, int userId, int roomId, RoomBanTypes length) {
        ...
        if (!rights.hasPermission(Permission.ACC_AMBASSADOR) && !room.hasRights(rights)) {
            return;
        }
    ...
    public int GetRoomsWithUsers() {
        int total = 0;
        for (Room room : this.activeRooms.values()) {
            if (room.getHabbos().size() > 0) {
                total++;
            }
        }
