...
public class RoomPromotion {
    ...
    public RoomPromotion(Room room, String title, String description, int endTimestamp, int startTimestamp, int category, int event) {
        this.room = room;
        this.title = title;
    ...
    public RoomPromotion(Room room, ResultSet set) throws SQLException {
        ...
        this.event = set.getInt("event");
    }

    public void save() {
        if (this.needsUpdate) {
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("UPDATE room_promotions SET title = ?, description = ?, category = ?, event = ? WHERE room_id = ?")) {
                statement.setString(1, this.title);
                statement.setString(2, this.description);
                statement.setInt(3, this.category);
                statement.setInt(4, this.event);
