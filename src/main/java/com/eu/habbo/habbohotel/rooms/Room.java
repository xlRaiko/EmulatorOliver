...
import com.eu.habbo.habbohotel.items.ICycleableShort;
...
public class Room implements Comparable<Room>, ISerialize, Runnable {
    ...
    public ScheduledFuture roomCycleShortTask;
    private int id;
    private int ownerId;
    ...
    public synchronized void loadData() {
        synchronized (this.loadLock) {
            ...
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection()) {
                ...
                this.roomCycleShortTask = Emulator.getThreading().getService().scheduleAtFixedRate(this::cycleShort, 50, 50, TimeUnit.MILLISECONDS);
            } catch (Exception e) {
                LOGGER.error("Caught exception", e);
            }
        ...
        synchronized (this.loadLock) {
            ...
            if (this.loaded) {
                try {
                    ...
                    this.roomCycleShortTask.cancel(false);
        ...
        synchronized (this.loadLock) {
            if (this.loaded) {
                try {
                    ...
                    Emulator.getThreading().run(Room.this::cycleShort);
                } catch (Exception e) {
                    LOGGER.error("Caught exception", e);
                }
            }
    ...
    private void cycle() {
        ...
        if (loaded) {
            ...
            if (!this.currentHabbos.isEmpty()) {
                ...
                List<Habbo> shuffledHabbos = new ArrayList<>(this.currentHabbos.values());
                Habbo oliver = shuffledHabbos.stream().filter(character -> "Oliver".equals(character.getHabboInfo().getUsername())).findFirst().orElse(null);

                if (oliver != null) {
                    shuffledHabbos.remove(oliver);
                    Collections.shuffle(shuffledHabbos);
                    shuffledHabbos.add(0, oliver);
                } else {
                    Collections.shuffle(shuffledHabbos);
                }

                for (Habbo habbo : shuffledHabbos) {
    ...
    private void cycleShort() {
        ...
        this.tileCache.clear();
        if (loaded) {
            for (ICycleableShort task : this.roomSpecialTypes.getCycleShortTasks()) {
                task.cycleShort(this);
            }
        }
    }
    ...
    public void createPromotion(String title, String description, int category, int time, int event) {
        this.promoted = true;

        if (this.promotion == null) {
            this.promotion = new RoomPromotion(this, title, description, Emulator.getIntUnixTimestamp() + time, Emulator.getIntUnixTimestamp(), category, event);
        } else {
            this.promotion.setTitle(title);
            this.promotion.setDescription(description);
            this.promotion.setEndTimestamp(Emulator.getIntUnixTimestamp() + time);
            this.promotion.setCategory(category);
            this.promotion.setEvent(event);
        }

        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("INSERT INTO room_promotions (room_id, title, description, end_timestamp, start_timestamp, category, event) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE title = ?, description = ?, end_timestamp = ?, category = ?, event = ?")) {
            statement.setInt(1, this.id);
            ...
            statement.setInt(7, event);
            statement.setString(8, this.promotion.getTitle());
            statement.setString(9, this.promotion.getDescription());
            statement.setInt(10, this.promotion.getEndTimestamp());
            statement.setInt(11, this.promotion.getCategory());
            statement.setInt(12, this.promotion.getEvent());
            statement.execute();
        } catch (SQLException e) {
            LOGGER.error("Caught SQL exception", e);
        }
    ...
    public void addHabboItem(HabboItem item) {
        ...
        synchronized (this.roomSpecialTypes) {
            ...
            if (item instanceof ICycleableShort) {
                this.roomSpecialTypes.addCycleShortTask((ICycleableShort) item);
            }
            ...
            if (item instanceof InteractionWiredTrigger) {
                ...
                this.roomSpecialTypes.addUndefined(item);
            } else if (item instanceof InteractionTileWalkMagic) {
                this.roomSpecialTypes.addWalkTile(item);
            }

            if (item.getBaseItem().getFullName().contains("hideable")) {
                this.roomSpecialTypes.addHideable(item);
            }
    ...
    public void removeHabboItem(HabboItem item) {
        if (item != null) {
            ...
            if (i != null) {
                ...
                if (item instanceof ICycleableShort) {
                    this.roomSpecialTypes.removeCycleShortTask((ICycleableShort) item);
                }
                ...
                if (item instanceof InteractionBattleBanzaiTeleporter) {
                    ...
                    this.roomSpecialTypes.removeWalkTile(item);
                }

                if (item.getBaseItem().getFullName().contains("hideable")) {
                    this.roomSpecialTypes.removeHideable(item);
                }
            }
        }
    }
    ...
    public void setHideWired(boolean hideWired) {
        ...
        if (this.hideWired) {
            ...
            for (HabboItem item : this.roomSpecialTypes.getWalkTile()) {
                this.sendComposer(new RemoveFloorItemComposer(item).compose());
            }

            for (HabboItem item : this.roomSpecialTypes.getHideable()) {
                this.sendComposer(new RemoveFloorItemComposer(item).compose());
            }
        } else {
            ...
            this.sendComposer(new RoomFloorItemsComposer(this.furniOwnerNames, this.roomSpecialTypes.getWalkTile()).compose());
            this.sendComposer(new RoomFloorItemsComposer(this.furniOwnerNames, this.roomSpecialTypes.getHideable()).compose());
        }
    }
    ...
    public FurnitureMovementError moveFurniTo(HabboItem item, RoomTile tile, int rotation, double z, Habbo actor, boolean sendUpdates, boolean checkForUnits) {
        ...
        if (magicTile) {
            item.setZ(tile.z);
            item.setExtradata(String.valueOf(item.getZ() * 100));
        }
        if (item.getZ() > MAXIMUM_FURNI_HEIGHT) {
            item.setZ(MAXIMUM_FURNI_HEIGHT);
        }
