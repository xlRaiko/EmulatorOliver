...
public class RoomTile {
    ...
    public boolean removeBuggedUnits() {
        boolean tileFixed = false;

        List<RoomUnit> tileUnits = this.getUnits();
        List<RoomUnit> buggedUnits = new ArrayList<>();

        for (RoomUnit roomUnit : tileUnits) {
            RoomTile currentTile = roomUnit.getCurrentLocation();

            if (currentTile.x != this.x || currentTile.y != this.y || currentTile.z != this.z) {
                tileFixed = true;
                buggedUnits.add( roomUnit );
            }
        }

        for (RoomUnit buggedUnit : buggedUnits) {
            this.removeUnit(buggedUnit);
        }
