...
import com.eu.habbo.core.HighscoreDelete;
import com.eu.habbo.core.HighscoreLog;
...
public class WiredHighscoreManager {
    ...
    public List<WiredHighscoreDataEntry> getHighscoreData(int itemId) {
    ...
    public void addHighscoreData(WiredHighscoreDataEntry entry) {
        ...
        this.data.get(entry.getItemId()).add(entry);
        Emulator.getThreading().run(new HighscoreLog(entry.getItemId(), entry.getUserIds().stream().map(Object::toString).collect(Collectors.joining(",")), entry.getScore(), entry.isWin(), entry.getTimestamp()));
    }

    public void removeHighscoreData(int itemId) {
        if (this.data.containsKey(itemId)) {
            this.data.get(itemId).clear();
            Emulator.getThreading().run(new HighscoreDelete(itemId));
        }
    }
    ...
    public List<WiredHighscoreRow> getHighscoreRowsForItem(int itemId, WiredHighscoreClearType clearType, WiredHighscoreScoreType scoreType) {
        ...
        if (scoreType == WiredHighscoreScoreType.MOSTWIN) {
            return highscores
                    .collect(Collectors.groupingBy(WiredHighscoreRow::getUsers, Collectors.summarizingInt(WiredHighscoreRow::getValue)))
                    .entrySet()
                    .stream()
                    .map(e -> new WiredHighscoreRow(e.getKey(), (int) e.getValue().getSum()))
                    .sorted(WiredHighscoreRow::compareTo)
                    .collect(Collectors.toList());
        }
