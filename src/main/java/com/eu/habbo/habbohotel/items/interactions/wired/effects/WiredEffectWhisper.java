...
public class WiredEffectWhisper extends InteractionWiredEffect {
    public static final String SET_SUPERWIRED_POINTS_KEY = "kb.superwired.points";
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        if (!this.message.isEmpty()) {
            if (roomUnit != null) {
                ...
                if (habbo != null) {
                    int points = 0;
                    if (habbo.getHabboStats().cache.get(SET_SUPERWIRED_POINTS_KEY) != null) {
                        points = (int) habbo.getHabboStats().cache.get(SET_SUPERWIRED_POINTS_KEY);
                    }

                    String msg = this.message.replace("%user%", habbo.getHabboInfo().getUsername()).replace("%online_count%", String.valueOf(Emulator.getGameEnvironment().getHabboManager().getOnlineCount())).replace("%room_count%", String.valueOf(Emulator.getGameEnvironment().getRoomManager().getActiveRooms().size())).replace("%points%", String.valueOf(points));
                ...
                for (Habbo h : room.getHabbos()) {
                    int points = 0;
                    if (h.getHabboStats().cache.get(SET_SUPERWIRED_POINTS_KEY) != null) {
                        points = (int) h.getHabboStats().cache.get(SET_SUPERWIRED_POINTS_KEY);
                    }

                    h.getClient().sendResponse(new RoomUserWhisperComposer(new RoomChatMessage(this.message.replace("%user%", h.getHabboInfo().getUsername()).replace("%online_count%", String.valueOf(Emulator.getGameEnvironment().getHabboManager().getOnlineCount())).replace("%room_count%", String.valueOf(Emulator.getGameEnvironment().getRoomManager().getActiveRooms().size())).replace("%points%", String.valueOf(points)), h, h, RoomChatMessageBubbles.WIRED)));
                }
