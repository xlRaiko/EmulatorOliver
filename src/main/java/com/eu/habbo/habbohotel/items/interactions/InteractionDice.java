        ...
        if (client != null) {
            if (RoomLayout.tilesAdjecent(room.getLayout().getTile(this.getX(), this.getY()), client.getHabbo().getRoomUnit().getCurrentLocation())) {
                if (!this.getExtradata().equalsIgnoreCase("-1")) {
                    int newResult = (Emulator.getRandom().nextInt(this.getBaseItem().getStateCount()) + 1);
                    FurnitureDiceRolledEvent event = (FurnitureDiceRolledEvent) Emulator.getPluginManager().fireEvent(new FurnitureDiceRolledEvent(this, client.getHabbo(), newResult));
                    ...
                    if (event.result > 0) {
                        ...
                        Emulator.getThreading().run(new RandomDiceNumber(this, room, this.getBaseItem().getStateCount(), newResult), 1500);
                    }
                }
            }
        }
