    ...
    public List<CatalogPage> getCatalogPages(int parentId, final Habbo habbo) {
        ...
        this.catalogPages.get(parentId).childPages.forEachValue(new TObjectProcedure<CatalogPage>() {
            ...
            public boolean execute(CatalogPage object) {
                ...
                boolean vipRightsOkay = true;

                if (object.isVipOnly() && !habbo.getHabboInfo().getHabboStats().hasActiveVip()) {
                    vipRightsOkay = false;
                }

                if (isVisiblePage && hasRightRank && clubRightsOkay && vipRightsOkay) {
                    pages.add(object);
                }
    ...
    public void purchaseItem(CatalogPage page, CatalogItem item, Habbo habbo, int amount, String extradata, boolean free) {
        ...
        try {
            ...
            try {
                ...
                int totalPixels = free ? 0 : this.calculateDiscountedPrice(item.getPixels(), amount, item);
                ...
                if (totalPixels > 0 && habbo.getHabboInfo().getPixels() - totalPixels < 0) return;
                ...
                UserCatalogItemPurchasedEvent purchasedEvent = new UserCatalogItemPurchasedEvent(habbo, item, itemsList, totalCredits, totalPixels, totalPoints, badges);
                ...
                if (!free && !habbo.getClient().getHabbo().hasPermission(Permission.ACC_INFINITE_PIXELS)) {
                    if (purchasedEvent.totalPixels > 0) {
                        habbo.getClient().getHabbo().givePixels(-purchasedEvent.totalPixels);
                    }
                }
