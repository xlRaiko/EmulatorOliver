...
public class CatalogPurchaseLogEntry implements Runnable, DatabaseLoggable {
    ...
    private static final String QUERY = "INSERT INTO `logs_shop_purchases` (timestamp, user_id, catalog_item_id, item_ids, catalog_name, cost_credits, cost_pixels, cost_points, points_type, amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public CatalogPurchaseLogEntry(int timestamp, int userId, int catalogItemId, String itemIds, String catalogName, int costCredits, int costPixels, int costPoints, int pointsType, int amount) {
    ...
    public void log(PreparedStatement statement) throws SQLException {
        ...
        statement.setInt(7, this.costPixels);
        statement.setInt(8, this.costPoints);
        statement.setInt(9, this.pointsType);
        statement.setInt(10, this.amount);
        statement.addBatch();
    }
