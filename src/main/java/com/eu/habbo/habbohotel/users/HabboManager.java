...
//
    ...
    public int verifyExits(int userId, int type) {
        int total = 0;

        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("SELECT count(*) AS total FROM users_currency WHERE user_id = ? AND type = ?")) {
            statement.setInt(1, userId);
    ...
    public void givePoints(int userId, int type, int amount) {
        Habbo habbo = this.getHabbo(userId);
        if (habbo != null) {
            habbo.givePoints(type, amount);
        } else {
            if (verifyExits(userId, type) > 0) {
                try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("UPDATE users_currency SET amount = amount + ? WHERE user_id = ? AND type = ? LIMIT 1")) {
                ...
                try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("INSERT INTO users_currency (user_id, type, amount) VALUES (?, ?, ?)")) {
                    statement.setInt(1, userId);
                    statement.setInt(2, type);
