...
public class Habbo implements Runnable {
    ...
    public boolean connect() {
        ...
        this.habboStats.cache.put("online", Emulator.getIntUnixTimestamp());
    ...
    public void deleteBadge(HabboBadge badge) {
        if (badge != null) {
            ...
            this.client.sendResponse(new RemoveUserBadgeComposer(badge));

            THashMap<String, String> keys = new THashMap<>();
            keys.put("display", "BUBBLE");
            keys.put("image", "${image.library.url}album1584/" + badge.getCode() + ".gif");
            keys.put("message", Emulator.getTexts().getValue("commands.generic.cmd_badge.removed"));
            this.client.sendResponse(new BubbleAlertComposer(BubbleAlertKeys.RECEIVED_BADGE.key, keys));
        }
    }
    ...
    public void respect(Habbo target) {
        if (target != null && target != this.client.getHabbo()) {
            ...
            this.client.getHabbo().getHabboInfo().getCurrentRoom().sendComposer(new RoomUsersComposer(target).compose());
