...
public class HabboStats implements Runnable {
    ...
    public int colorId;
    public int usernameIconId;
    public int usernameEffectId;
    public int kissesReceived;
    ...
    private HabboStats(ResultSet set, HabboInfo habboInfo) throws SQLException {
        ...
        this.bannerId = set.getInt("banner_id");
        this.bannerColor = set.getString("banner_color");
        this.ornamentId = set.getInt("ornament_id");
        this.colorId = set.getInt("color_id");
        this.usernameIconId = set.getInt("username_icon_id");
        this.usernameEffectId = set.getInt("username_effect_id");
        this.kissesReceived = set.getInt("kisses_received");
        this.profileParameters = set.getString("profile_parameters");
    ...
    public void run() {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE users_settings SET achievement_score = ?, respects_received = ?, respects_given = ?, daily_respect_points = ?, block_following = ?, block_friendrequests = ?, online_time = online_time + ?, guild_id = ?, daily_pet_respect_points = ?, club_expire_timestamp = ?, login_streak = ?, rent_space_id = ?, rent_space_endtime = ?, volume_system = ?, volume_furni = ?, volume_trax = ?, block_roominvites = ?, old_chat = ?, block_camera_follow = ?, chat_color = ?, hof_points = ?, block_alerts = ?, talent_track_citizenship_level = ?, talent_track_helpers_level = ?, ignore_bots = ?, ignore_pets = ?, nux = ?, mute_end_timestamp = ?, allow_name_change = ?, perk_trade = ?, can_trade = ?, `forums_post_count` = ?, ui_flags = ?, has_gotten_default_saved_searches = ?, max_friends = ?, max_rooms = ?, last_hc_payday = ?, hc_gifts_claimed = ?, banner_id = ?, banner_color = ?, ornament_id = ?, color_id = ?, username_icon_id = ?, username_effect_id = ?, kisses_received = ?, profile_parameters = ? WHERE user_id = ? LIMIT 1")) {
                ...
                statement.setInt(39, this.bannerId);
                statement.setString(40, this.bannerColor);
                statement.setInt(41, this.ornamentId);
                statement.setInt(42, this.colorId);
                statement.setInt(43, this.usernameIconId);
                statement.setInt(44, this.usernameEffectId);
                statement.setInt(45, this.kissesReceived);
                statement.setString(46, this.profileParameters);
                statement.setInt(47, this.habboInfo.getId());
    ...
    public int getVipExpireTimestamp() {
        return getSubscriptionExpireTimestamp(Subscription.HABBO_VIP);
    }
    ...
    public void setVipExpireTimestamp(int clubExpireTimestamp) {
        Subscription subscription = getSubscription(Subscription.HABBO_VIP);
        int duration = clubExpireTimestamp - Emulator.getIntUnixTimestamp();
        ...
        if(duration > 0) {
            createSubscription(Subscription.HABBO_VIP, duration);
        }
    }
    ...
    public boolean hasActiveVip() {
        return hasSubscription(Subscription.HABBO_VIP);
    }
    ...
    public int getPastTimeAsVip() {
        int pastTimeAsHC = 0;
        for(Subscription subs : this.subscriptions) {
            if(subs.getSubscriptionType().equalsIgnoreCase(Subscription.HABBO_VIP)) {
                pastTimeAsHC += subs.getDuration() - (Math.max(subs.getRemaining(), 0));
            }
        }
        return pastTimeAsHC;
    ...
    public void setBlockFriendRequests(boolean blockFriendRequests) {
    ...
    public void setOrnament(int ornamentId) {
    ...
    public int getUsernameIconId() {
        return this.usernameIconId;
    }

    public void setUsernameIconId(int usernameIconId) {
        this.usernameIconId = usernameIconId;
    }

    public int getUsernameEffectId() {
        return this.usernameEffectId;
    }

    public void setUsernameEffectId(int usernameEffectId) {
        this.usernameEffectId = usernameEffectId;
    }
    ...
    public void setKissesReceived(int kissesReceived) {
    ...
    public void setProfileParameters(String profileParameters) {
